...
def osd_sink_pad_buffer_probe(pad,info,u_data):
    ...
    with np.load('/app/models/embeddings.npz') as embeddings:
        for key in embeddings.keys():
            emb_array.append(embeddings[key])


    with open("/app/models/names.txt") as f:
        lines = f.readlines()
        for line in lines:
...
####################
    ...
    pgie.set_property('config-file-path', "/app/configs/deepstream_pgie_config.txt")
    sgie1.set_property('config-file-path', "/app/configs/deepstream_sgie1_config.txt")

    #Set properties of tracker
    config = configparser.ConfigParser()
    config.read('/app/configs/deepstream_tracker_config.txt')
    config.sections()
